...
ErrorCode_t Game::create_static_object()
{
    ...
    {
        LoaderParams params = LoaderParams(GROUND_POSITION - 300, 20, 300, 300, eTEXTURE_TREE_FORM_1);
...
ErrorCode_t Game::create_dynamic_object()
{
    ...
    const int tree_base_position_y = 10 + y_offset;
    const int x_offset = 60;
    const int tree_base_position_x = GROUND_POSITION - 300 + x_offset;
    ...
    for (int i = 0; i < 10; i++)
    {
        ...
        {
            y_fruit_offset += rand() % 32 + 32;
            if (y_fruit_offset >= 200)
            {
                x_fruit_offset = x_fruit_offset + 35;
                y_fruit_offset = 0;
            }
            else
            {
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        {
            LoaderParams params = LoaderParams(400, 700 + i * 30, 0, 0, eTEXTURE_BIRDS);
